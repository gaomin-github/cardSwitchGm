            ...
            <div class="content" ref="container" @mousedown.stop="dragStartHandler" @mouseup="dragEndHandler" @mousemove.stop="dragMoveHandler">
                <template v-if="curList&&curList.length>0" >
                    ...
                    <div class="cards" :style="{transform:`translateX(${transformX}px`}" >
                        <div v-for="(item,index) in curList" :key="item.index" class="cards-item" @mouseenter="handleMouseOver(item,index)" :ref="`pic_${item.index}`" @click="handleEnterCard(item)">
...
<script>
    ...
    export default{
        ...
        computed:{
            ...
            containerRc(){
                if(this.containerRcStr){
                    return this.containerRcStr.split(',')
                }
        ...
        methods:{
            _getContainerRc(){
                if(this.$refs.container){
                    ...
                    // 左右边界位置
                    this.containerRcStr=`${cr.left},${cr.right}`;
                }
            },    
