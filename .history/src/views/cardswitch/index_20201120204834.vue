                ...
                <template v-if="curList&&curList.length>0" >
                    ...
                    <div class="cards" :style="{transform:`translateX(${transformX}px`}" >
                        <div v-for="(item,index) in curList" :key="item.index" class="cards-item" @mouseenter="handleMouseOver(item,index)" @mouseleave="handleMouseLeave" :ref="`pic_${item.index}`" @click="handleEnterCard(item)">
                            <img :class="['cards-item-pic',actCardId==item.index?'cards-item-pic-active':'']" v-lazy:imgData="{imgData:item,method:_loadImg,key:pageKey}" :alt="item.title"/>
...
<script>
    ...
    export default{
        ...
        mounted(){
            this._initContainerRc()
            // document.onmouseup=()=>{

            // }
            window.addEventListener('resize',()=>{
                // console.log(this.pageKey,'pageKey')
                // this.pageKey=Math.random();
                // intersectionObserver.observe()
                // this.canDrag=false;
                // console.log('mouseup',new Date().getTime())
                // (this._throttle(this._initContainerRc,50))()
                this._initContainerRc()
            })
        },
        methods:{
            _initContainerRc(){
                ...
                if(this.$refs.container){
                    ...
                    // 左右边界位置
                    // this.containerRc.splice(0,2,[cr.left,cr.right])
                    this.containerRc=[cr.left,cr.right]
                }
            },
            handleMouseLeave(){
            ...
            _loadImg(el,imgData){
                ...
                if(elRc.left<this.containerRc[1]-20){
                    console.log(imgData.index,'loading',182)
                    el.setAttribute('src',imgData.cardUrl)
                    el.setAttribute('loaded',true)
                }else{
                    console.log(this.containerRc,'container')
                    console.log(elRc.left,'elRc.left')
